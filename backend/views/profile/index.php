...
$filter_catnews =CatProfile::getAllCatProfileDDL();
        ...
        <?= GridView::widget([
            ...
            'columns' => [
                ...
                [
                    'header'=>'Ảnh đại diện',
                    'value'=>function ($data) {
                        return Html::img(Yii::$app->urlManagerFrontend->baseUrl.'/'.($data->path), ['title' => $data->title, 'alt' => $data->title, 'width' => 40]);
                    },
                    'format' => 'raw',
                ...
                [
                    'attribute'=>'desired_job_id',
                    'filter'=>$filter_catnews,
                    'value'=>function ($data) use ($filter_catnews) {
                        return Html::dropDownList('desired_job_id', $data->desired_job_id, $filter_catnews, [
                            "prompt" => "Vui lòng chọn...",
                            "data-attr" => "desired_job_id",
                            "class" => "cajax"
                        ]);
                    },
                    'format' => 'raw',
