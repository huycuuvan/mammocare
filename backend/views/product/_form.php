            ...
            <div class="white_bg">
                <div class="container-fluid">
                    <div class="row">
                        ...
                        <?php $timestamp = time(); ?>
                        <?php echo $form->field($model, 'pid', ['template' => '{input}'])->hiddenInput(['id'=>'pid','value'=> (($model->id)?$model->id:$timestamp)])->label(false); ?>
<!--                        --><?php //echo $form->field($model, 'pid', ['template' => '{input}'])->hiddenInput(['value'=> (($model->id)?$model->id:$timestamp)])->label(false); ?>
                    ...
                    <div class="row ">
                        ...
                        <div class="col-md-4">
                            <?= $form->field($model, 'description2', ['enableClientValidation' => false])->textInput(); ?>
                        </div>
                        <div class="col-12 col-md-4">
                            <?= $form->field($model, 'description3', ['enableClientValidation' => false])->textInput(); ?>
                        </div>
                    </div>
                    <div class="row d-none">

                        <div class="col-12">
                            <?= $form->field($model, 'description4', ['enableClientValidation' => false])->textarea(['rows' => 4]) ; ?>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-12 ">
                            <?= $form->field($model, 'brief', ['enableClientValidation' => false])->textarea(['rows' => 6, 'class' => 'ckeditor']) ?>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-12">
                            <pre>Lưu ý: Chi tiết bài viết Tạo nút hotline với cấu trúc {{{hotline}}} , Tạo nút chat zalo với cấu trúc {{số_điện_thoại}} </pre>
                                ...
                                foreach ($imgs as $row) {
                                    ...
                                    <li data-id="<?=$row->id; ?>">
                                        ...
                                        <span>
                                            ...
                                            <a class="add_youtube" href="#" title="Mã Youtube"><i class="fab fa-youtube-square"></i></a>
                                        </span>
                                    </li>
                                    <?php
                                }
...
//                            }
            ...
            <div class="white_bg d-none">
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-6"><strong class="title_box">Tiện ích</strong></div>
                    ...
                    <div id="relatedNews" class="related_box">
                        ...
                        <div class="row no-gutters search_rbox d-none" id="searchForm">
                            <div class="col-8 form-group">
                                <input class="form-control" autocomplete="off" type="text" name="title" placeholder="Từ khóa tìm kiếm...">
                            </div>
                            <div class="col-4 form-group">
                                <?=Html::dropDownList('catnews_id', '', Brand::getBrandDDL(), ["class" => "form-control", 'prompt' => 'Chọn danh mục...']); ?>
                            </div>
                        </div>
        ...
        <div class="col-md-4">
            ...
            <div class="white_bg vina-right">
                <div class="container-fluid">
                    ...
                    if (!empty($brandList)): ?>
                        ...
                        <div class="row line line_ddl ">
                            <div class="col-12">
                                <?= $form->field($model, 'brand_id')->dropDownList($brandList, ['class' => 'mulselect_cat', 'prompt' => 'Vui lòng chọn...', 'multiple' => 'multiple']); ?>
                            </div>
                        </div>
                    <?php endif;
                    $placeList = Place::getPlaceDDL();
                    if (!empty($placeList)): ?>
                        <div class="row line line_ddl">
                            <div class="col-12">
                                <?= $form->field($model, 'place_id')->dropDownList($placeList, ['prompt' => 'Vui lòng chọn...']); ?>
                            </div>
                        </div>
                    <?php endif;
                    ?>
            ...
            ?>
                <div class="white_bg vina-right d-none">
                    <div class="container-fluid">
                        ...
                        $class='';
                            ?>
                            <div class="row line line_ddl <?=$class?> property-cat" data-id="<?=$row->id?>" >
                                <div class="col-12">
                                    <?= $form->field($model, "property[{$row->id}]")->dropDownList(PropertyValue::getPropertyValueDDL($row->id), ['class' => "mulselect_property ", 'multiple' => 'multiple'])->label($row->name); ?>
                                </div>
                            </div>
                        <?php endforeach; ?>
                        <span class="save-prd d-none" data-content="<?= Url::toRoute(['product/save-product']) ?>"></span>
                        <a href="<?= Url::toRoute(['product/save-new']) ?>" class="btn d-block mb-2 btn-info rounded-sm submit_redirect_image">Phân loại sản phẩm</a>
                        <a href="<?= Url::toRoute(['product/save-price']) ?>" class="btn d-none btn-info rounded-sm submit_redirect_pricec">Bảng giá sản phẩm</a>
                        <?= $form->field($model, 'property_price')->hiddenInput(['maxlength' => true, 'id' => 'property_price'])->label(false); ?>
                        <?= $form->field($model, 'property_list_item')->hiddenInput(['maxlength' => true, 'id' => 'property_list_item'])->label(false); ?>
                    </div>
                </div>
            <?php endif; ?>
    ...
    <div class="modal fade bd-example-modal-lg" id="price-modal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLongTitle">Bảng giá sản phẩm</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
