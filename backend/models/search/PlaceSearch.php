...
class PlaceSearch extends Place
{
    ...
    {
        $query = Place::find()->select(['{{place}}.*', 'REPLACE({{place}}.path, "/'.parent::tableName().'/", "/'.parent::tableName().'/thumb/") AS thumb'])
        ->joinWith('language')
        ...
        $query->andFilterWhere([
            '{{place}}.id' => $this->id,
            '{{place}}.ord' => $this->ord,
            '{{place}}.active' => $this->active,
            'home' => $this->home,
        ]);

        $query->andFilterWhere(['like', '{{place}}.name', $this->name])
